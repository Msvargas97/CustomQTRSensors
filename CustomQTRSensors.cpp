...
//Alamacena la ultima calibracion realizada y los guarda en la memoria EEPROM
void QTRSensors::saveCalibration()
{
	int i=0;
	if(readModeCalibrate == QTR_EMITTERS_ON_AND_OFF || readModeCalibrate == QTR_EMITTERS_ON){
	if(calibratedMinimumOn){
		for (i=0;i<_numSensors;i++)
		{
		 EEPROM.put(i*4,calibratedMaximumOn[i]);
		 EEPROM.put((i*4)+2,calibratedMinimumOn[i]);
		}
	 }
	}
    if(readModeCalibrate == QTR_EMITTERS_ON_AND_OFF || readModeCalibrate == QTR_EMITTERS_OFF)
    {
	if(calibratedMinimumOff){
		for (i=0;i<_numSensors;i++)
		{
		 EEPROM.put(i*4,calibratedMaximumOff[i]);
		 EEPROM.put((i*4)+2,calibratedMinimumOff[i]);
		}
	 }
	}
 EEPROM.put((_numSensors*4)+2,readModeCalibrate);	
}
//Resablece los valores calibrados con anterioridad
void QTRSensors::restoreCalibration()
{
	int i;
	EEPROM.get((_numSensors*4)+2,readModeCalibrate);
	QTRSensors::calibrate(readModeCalibrate);
	
	if(readModeCalibrate == QTR_EMITTERS_ON_AND_OFF || readModeCalibrate == QTR_EMITTERS_ON){
	if(calibratedMinimumOn){
	for (i=0;i<_numSensors;i++)
	{
		EEPROM.get(i*4,calibratedMaximumOn[i]);
		EEPROM.get((i*4)+2,calibratedMinimumOn[i]);
	}
	}
	}
	    if(readModeCalibrate == QTR_EMITTERS_ON_AND_OFF || readModeCalibrate == QTR_EMITTERS_OFF)
	...
	if(calibratedMinimumOff){
		for (i=0;i<_numSensors;i++)
		{
		EEPROM.get(i*4,calibratedMaximumOff[i]);
		EEPROM.get((i*4)+2,calibratedMinimumOff[i]);
		}
	 }
	}
}
...
int QTRSensors::readLine(unsigned int *sensor_values,
    unsigned char readMode, unsigned char white_line,int noise,int MAX)
{
